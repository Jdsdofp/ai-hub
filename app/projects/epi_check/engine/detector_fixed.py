...
class EPIEngine:
    ...
    def generate_dataset(self, company_id: int, train_split: float = 0.8) -> dict:
        ...
        if not pairs:
            return {"error": "No valid annotated images found. Upload photos (Upload tab), annotate them (Annotate tab), then generate the dataset."}
        ...
        yaml_data = {
            "train": str(CompanyData.epi(company_id, "dataset", "train", "images").resolve()),
            "val": str(CompanyData.epi(company_id, "dataset", "valid", "images").resolve()),
