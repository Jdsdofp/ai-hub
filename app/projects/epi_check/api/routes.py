...
# # ======================================================================
# # EDGE DEVICES — Endpoints (colar no routes.py ou em arquivo separado)
# # ======================================================================
...
# @router.post("/device/register", tags=["Edge Devices"], summary="Register or Update Edge Device")
# async def register_device(data: EdgeDeviceRegister, company_id: int = Depends(get_ui_company)):
#     try:
#         device_id = await repo.register_edge_device(
#             company_id=company_id,
...
#         }
#     except Exception as e:
#         logger.error(f"[Company {company_id}] register_device error: {e}")
#         raise HTTPException(500, detail=f"Failed to register device: {str(e)}")


# # POST /device/heartbeat
# @router.post("/device/heartbeat", tags=["Edge Devices"], summary="Device Heartbeat Ping")
# async def device_heartbeat(device_code: str = Form(...), company_id: int = Depends(get_ui_company)):
#     try:
#         ok = await repo.heartbeat_edge_device(company_id, device_code)
#         if not ok:
#             raise HTTPException(404, detail=f"Device not found: {device_code}")
#         return {"alive": True, "device_code": device_code, "ts": datetime.utcnow().isoformat()}
#     except HTTPException:
#         raise
#     except Exception as e:
#         logger.error(f"[Company {company_id}] device_heartbeat error: {e}")
#         raise HTTPException(500, detail=f"Heartbeat failed: {str(e)}")


# # GET /device/list
# @router.get("/device/list", tags=["Edge Devices"], summary="List Edge Devices")
# async def list_devices(
#     active_only: bool = Query(True),
#     company_id: int = Depends(get_ui_company),
# ):
#     try:
#         return await repo.list_edge_devices(company_id, active_only)
#     except Exception as e:
#         logger.error(f"[Company {company_id}] list_devices error: {e}")
#         raise HTTPException(500, detail=f"Failed to list devices: {str(e)}")


# # DELETE /device/{device_code}
# @router.delete("/device/{device_code}", tags=["Edge Devices"], summary="Deactivate Edge Device")
# async def deactivate_device(device_code: str, company_id: int = Depends(get_ui_company)):
#     try:
#         ok = await repo.deactivate_edge_device(company_id, device_code)
#         if not ok:
#             raise HTTPException(404, detail=f"Device not found: {device_code}")
#         return {"success": True, "device_code": device_code, "active": False}
#     except HTTPException:
#         raise
#     except Exception as e:
#         logger.error(f"[Company {company_id}] deactivate_device error: {e}")
#         raise HTTPException(500, detail=f"Failed to deactivate device: {str(e)}")
