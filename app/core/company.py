...
"""
Company data isolation. Structure: data/{company_id}/{project}/...
All paths are resolved to absolute to avoid YOLO/Ultralytics path conflicts.
"""
import os
from pathlib import Path
from app.core.config import settings
...
class CompanyData:
    ...
    @classmethod
    def _root(cls) -> Path:
        """Always returns absolute DATA_ROOT path."""
    ...
    @classmethod
    def get_project_root(cls, company_id: int, project: str = "epi_check") -> Path:
        p = cls._root() / str(company_id) / project
        if not p.exists():
            cls._create_project_dirs(company_id, project)
        return p
    ...
    @classmethod
    def _create_project_dirs(cls, company_id: int, project: str):
        base = cls._root() / str(company_id) / project
        if project == "epi_check":
            for sub in cls.EPI_SUBDIRS:
                (base / sub).mkdir(parents=True, exist_ok=True)
        else:
